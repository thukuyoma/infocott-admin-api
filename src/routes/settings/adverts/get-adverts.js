...
router.get('/adverts', checkAuthToken, checkValidAdmin, async (req, res) => {
  const { limit } = req.query;
  const page = Number(req.query.page);
  const pageLimit = Number(limit);
  const { db } = await connectToDatabase();
  const documentCount = await db.collection('adverts').countDocuments({});

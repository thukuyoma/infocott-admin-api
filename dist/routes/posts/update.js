...
router.put('/edit/:postId', _checkToken.default, (0, _uploader.default)('/images/posts/').single('image'), async (req, res) => {
  ...
  const postToEdit = await db.collection('posts').findOne({
  ...
  const path = file ? file.path : ''; //validate post
